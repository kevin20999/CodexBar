...
## 0.18.0-beta.4 — 2026-04-16
### Highlights
- Reframe the app as **CodexTokenBar**, a Codex / OpenAI-focused fork with a new token dashboard, quota surfaces, and release flow tuned for GitHub Releases.
- Add the new **TokenBar** interface: quota cards, detailed token overview, compact numeric summary, recent 30-day and 48-hour charts, and human instruction activity tracking.
- Add the **CodexDaily** companion app and shared token-history engine while keeping CodexTokenBar as the primary public app bundle.
- Add first-class Codex token history, Spark quota, OpenAI dashboard scraping, browser-cookie gating, and shared usage refresh infrastructure.

### Product & UI
- Introduce a new `TokenApp` menu bar experience with configurable dashboard modules, richer menu content, detailed charts, and numeric usage summaries.
- Add a popup style preference for the main menu (`液态玻璃` or `系统 popover`) while preserving the current liquid-glass presentation as the default.
- Rework quota cards, charts, and usage overview cards for clearer token, quota, and main-thread visibility.
- Remove hover-only affordances from the detailed overview where they were adding noise instead of value.

### Token history & quota
- Add a dedicated token history engine, Codex session parsing, refresh scheduling, and shared presentation caching for smoother updates.
- Switch Spark quota retrieval to the same structured Codex usage API path used for primary Codex quota, with webpage scraping only as fallback.
- Align CodexDaily token history generation and refresh cadence with CodexTokenBar while keeping separate caches.
- Add main-thread scoped charts and summaries for recent 30 days, recent 48 hours, and overview totals.

### Packaging & release
- Rename packaging and runtime paths around `CodexTokenBar` and `CodexDaily`, including dedicated package/run scripts for each app.
- Update signing, notarization, appcast, and GitHub Release scripts to work with the forked repository and CodexTokenBar naming.
- Decouple `sign-and-notarize.sh` from Sparkle so a notarized GitHub Release zip can be produced without appcast setup.
- Add release documentation for first-time GitHub Release publishing from a personal fork.

### Docs & repo
- Rewrite the README around the CodexTokenBar product focus and document upstream attribution for the fork.
- Add UI scheme and release setup documentation for the new TokenBar experience and GitHub Release workflow.
- Refresh development and packaging docs to cover CodexTokenBar/CodexDaily packaging and launch commands.
