...
public struct OpenAIDashboardBrowserCookieImporter {
    ...
    private static let cookieImportOrder: BrowserCookieImportOrder = Browser.defaultImportOrder
        ...
        logger: ((String) -> Void)? = nil) async throws -> ImportResult
    {
        ...
        if let cached = CookieHeaderCache.load(),
        ...
        // Match upstream behavior: prefer the provider's default order, which tries Safari first.
        ...
        try self.throwImportFailure(diagnostics: diagnostics, log: log)
    }

    public func importSafariCookies(
        intoAccountEmail targetEmail: String?,
        allowAnyAccount: Bool = false,
        logger: ((String) -> Void)? = nil) async throws -> ImportResult
    {
        ...
        if normalizedTarget != nil {
            log("Codex email known; Safari matching required.")
        } else {
            guard allowAnyAccount else {
                throw ImportError.noCookiesFound
            }
            log("Codex email unknown; importing any Safari signed-in session.")
        }

        var diagnostics = ImportDiagnostics()
        if let match = await self.tryBrowser(
            .safari,
            targetEmail: normalizedTarget,
            allowAnyAccount: allowAnyAccount,
            sourceLabelOverride: "Safari",
            log: log,
            diagnostics: &diagnostics)
        {
            return match
        ...
        diagnostics: inout ImportDiagnostics) async -> ImportResult?
    {
        do {
            ...
            for source in sources {
                ...
                let candidate = Candidate(label: sourceLabelOverride ?? source.label, cookies: cookies)
    ...
    private func throwImportFailure(
        diagnostics: ImportDiagnostics,
        log: @escaping (String) -> Void) throws -> Never
    {
    ...
    private func cacheCookies(candidate: Candidate) {
        ...
        CookieHeaderCache.store(cookieHeader: header, sourceLabel: candidate.label)
    }
