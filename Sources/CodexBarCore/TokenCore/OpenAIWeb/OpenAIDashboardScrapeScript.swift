...
let openAIDashboardScrapeScript = """
    ...
    (() => {
      ...
      const matchesCreditsRemainingHeading = (text) => {
        const lower = String(text || '').trim().toLowerCase();
        if (!lower) return false;
        return (
          lower === 'credits remaining' ||
          (lower.includes('credits') && lower.includes('remaining')) ||
          text.includes('剩余 Credits') ||
          text.includes('Credits 剩余') ||
          text.includes('残り Credits') ||
          text.includes('Credits 残り') ||
          text.includes('剩余额度')
        );
      };
      const matchesUsageBreakdownHeading = (text) => {
        const lower = String(text || '').trim().toLowerCase();
        if (!lower) return false;
        return (
          lower.startsWith('usage breakdown') ||
          text.includes('用量明细') ||
          text.includes('使用情况明细') ||
          text.includes('Usage breakdown') ||
          text.includes('利用内訳')
        );
      };
      const matchesCreditsHistoryHeading = (text) => {
        const lower = String(text || '').trim().toLowerCase();
        if (!lower) return false;
        return (
          lower === 'credits usage history' ||
          text.includes('Credits usage history') ||
          text.includes('Credits 历史') ||
          text.includes('Credits 使用记录') ||
          text.includes('Credits 使用历史') ||
          text.includes('クレジット利用履歴')
        );
      };
      const purchaseTextMatches = (text) => {
        ...
        if (
          ...
          original.includes('购买') ||
          original.includes('充值') ||
          original.includes('追加')
        ) {
          return true;
        }
        if (!lower.includes('credit') && !original.includes('Credits')) return false;
        return (
          lower.includes('buy') ||
          lower.includes('add') ||
      ...
      const findCreditsPurchaseButton = () => {
        ...
        const labelMatch = nodes.find(node => matchesCreditsRemainingHeading(textOf(node)));
      ...
      const usageBreakdownJSON = (() => {
        try {
          ...
          const usageSection = sections.find(s => {
            ...
            return h2 && matchesUsageBreakdownHeading(textOf(h2));
          });
      ...
      try {
        ...
        const header = headings.find(h => matchesCreditsHistoryHeading(textOf(h)));
        if (header) {
