---
summary: "CodexTokenBar release checklist: package, sign, notarize, appcast, and asset validation."
read_when:
  - Starting a CodexTokenBar release
  - Updating signing/notarization or appcast steps
  - Validating release assets or Sparkle feed
---

# Release process (CodexTokenBar)

SwiftPM-only; package/sign/notarize manually (no Xcode project). Sparkle feed is served from GitHub Releases. Checklist below merges Trimmy’s release flow with CodexTokenBar specifics.

**First time doing a GitHub Release?** Read [`docs/GITHUB_RELEASE_FIRST_TIME.md`](./GITHUB_RELEASE_FIRST_TIME.md) first. It covers the narrower “CodexTokenBar-only, GitHub Release first, Sparkle later” path.

**Must read first:** open the master macOS release guide at `~/Projects/agent-scripts/docs/RELEASING-MAC.md` alongside this file and reconcile any differences in favor of CodexTokenBar specifics before starting a release.

## Expectations
- When someone says “release CodexTokenBar”, do the entire end-to-end flow: bump versions/CHANGELOG, build, sign and notarize, upload the zip to the GitHub release, generate/update the appcast with the new signature, publish the tag/release, and verify the enclosure URL responds with 200/OK and installs via Sparkle (no 404s or stale feeds).
...
- Full `Scripts/release.sh` still expects both notarization creds and Sparkle tooling: `swiftformat`, `swiftlint`, `swift`, `sign_update`, `generate_appcast`, `gh`, `python3`, `zip`, `curl`, plus `APP_STORE_CONNECT_*` and `SPARKLE_PRIVATE_KEY_FILE`.

## Prereqs
- Xcode 26+ installed at `/Applications/Xcode.app` (for ictool/iconutil and SDKs).
- Your own Developer ID Application cert installed. Set `CODEXBAR_SIGNING_IDENTITY` (or `APP_IDENTITY`) to that identity before signing.
- ASC API creds in env: `APP_STORE_CONNECT_API_KEY_P8`, `APP_STORE_CONNECT_KEY_ID`, `APP_STORE_CONNECT_ISSUER_ID`.
- `SPARKLE_PRIVATE_KEY_FILE` is only required if you are also generating an appcast entry.
- If Releases/appcast are not hosted on the current `origin` repo, set `CODEXBAR_RELEASE_REPO` and/or `CODEXBAR_APPCAST_FEED_URL`.
...
- Packages `.build/apps/CodexTokenBar.app` with Info.plist and Icon.icns
- Codesigns **everything** with runtime + timestamp (deep) and adds rpath
- Zips to `CodexTokenBar-<version>.zip`
...
`SPARKLE_PRIVATE_KEY_FILE` is **not** required here; Sparkle is only needed for appcast generation and verification.

Gotchas fixed:
- Use `--timestamp` and `--deep` when signing the app to avoid invalid signature errors.
- Avoid `unzip` — it can add AppleDouble `._*` files that break the sealed signature and trigger “app is damaged”. Use Finder or `ditto -x -k CodexTokenBar-<ver>.zip /Applications`. If Gatekeeper complains, delete the app bundle, re-extract with `ditto`, then `spctl -a -t exec` to verify.
- Manual sanity check before uploading: `find .build/apps/CodexTokenBar.app -name '._*'` should return nothing; then `spctl --assess --type execute --verbose .build/apps/CodexTokenBar.app` and `codesign --verify --deep --strict --verbose .build/apps/CodexTokenBar.app` should both pass on the packaged bundle.

## Optional Appcast (Sparkle)
After notarization:
```
SPARKLE_PRIVATE_KEY_FILE=/path/to/ed25519-priv.key \
./Scripts/make_appcast.sh CodexTokenBar-0.1.0.zip \
...
```
Uploads not handled automatically—commit/publish appcast + zip to the feed location (GitHub Releases/raw URL). This is optional for the current friend-distribution zip and only matters if you also maintain an update feed.
...
## Homebrew (Cask)
CodexTokenBar ships a Homebrew **Cask** in `../homebrew-tap`. Homebrew installs should be updated via `brew`; do not assume a separate in-app updater is present in the current `TokenApp` bundle.
...
- [ ] Optional: generate Sparkle appcast with private key
  ...
  - Generate the appcast + HTML release notes: `./Scripts/make_appcast.sh CodexTokenBar-<ver>.zip https://raw.githubusercontent.com/<owner>/<repo>/main/appcast.xml`
...
- [ ] Homebrew tap: update `../homebrew-tap/Casks/codexbar.rb` (url + sha256) and `../homebrew-tap/Formula/codexbar.rb` (Linux CLI tarball urls + sha256), then verify:
  ...
  - `brew install --cask steipete/tap/codexbar && open -a CodexTokenBar`
...
- [ ] Release pages: title format `CodexTokenBar <version>`, notes as Markdown list (no stray blank lines)
- [ ] Changelog/release notes are user-facing: avoid internal-only bullets (build numbers, script bumps) and keep entries concise
- [ ] Download uploaded `CodexTokenBar-<ver>.zip`, unzip via `ditto`, run, and verify signature (`spctl -a -t exec -vv CodexTokenBar.app` + `stapler validate`)
...
- [ ] Keep a previous signed build in `/Applications/CodexTokenBar.app` to test Sparkle delta/full update to the new release
- [ ] Manual Gatekeeper sanity: after packaging, `find .build/apps/CodexTokenBar.app -name '._*'` is empty, `spctl --assess --type execute --verbose .build/apps/CodexTokenBar.app` and `codesign --verify --deep --strict --verbose .build/apps/CodexTokenBar.app` succeed
- [ ] For Sparkle verification: if replacing `/Applications/CodexTokenBar.app`, quit first, replace, relaunch, and test update
